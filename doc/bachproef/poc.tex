...
Een andere beperkingen is dat beveiligingsmaatregelen zoals AppArmor en een firewall niet standaard ge\"installeerd zijn op Debian.
...
Alvast een overzicht van de verschillende servers die deel zullen uitmaken van de omgeving, samen met de roll is te vinden in tabel~\ref{table:poc-server-roles}.
...
De Bourne Again Shell (Bash)~\autocite{gnu-bash-home} is een veelgebruikte shell, ookwel command language interpreter genoemd, die oorspronkelijk is ontwikkeld door de Free Software Foundation onder het GNU-project.
